...
0. 먼저 **편집자 모드**를 켭니다: 설정 화면 맨 아래 버전 표시(`OPIc Coach v…`)를 7번 연속으로 누르세요.
   (평소에는 저장소 링크가 전부 숨겨져 있어서, 앱을 쓰는 다른 사람에게는 보이지 않습니다. 끄려면 설정 →
   편집자 모드 → **🔒 편집자 모드 끄기**. 이 설정은 그 기기 브라우저에만 저장됩니다.)
